...
async function runQuests(int, uid, tokens, value) {
    ...
    // Enroll one at a time with delay (avoid rate limit)
    for (const tkn of uniqTokens) {
        const qs = await fetchQuests(tkn); if (!qs) continue;
        for (const q of qs.filter(q => questIsPlay(q) && !(q.user_status || {}).enrolled_at && !questExpired(q))) {
            await questEnroll(tkn, q.id);
            await delay(800);
        }
    }
    ...
    while (!allDone) {
        ...
        if (runtime.stopFlags.get(uid)) {
            ...
            // Sequential claims on stop (avoid rate limit)
            (async () => {
                for (const st of states.filter(s => s.done)) {
                    await questClaim(st.tkn, st.qid);
                    await delay(300);
                }
        ...
        allDone = true;
        // Process heartbeats one at a time per token to avoid rate limits
        for (const st of states) {
            if (st.done) continue;
            if (st.target > 0 && st.cur >= st.target) { st.done = true; continue; }
            ...
            if (s === 200 && d?.progress) {
                ...
                if (p) {
                    ...
                    if (p.completed_at || st.cur >= st.target) { st.cur = st.target; st.done = true; continue; }
                }
            }
            ...
            if (pct >= 99) {
                ...
                if (verifyQ) {
                    ...
                    if (verifyQ.user_status?.claimed_at || vCur >= vTgt || vPct >= 100) {
                        st.cur = vTgt; st.done = true; continue;
                    }
                    st.cur = vCur;
                    st.staleCount = 0;
                    ...
                    st.cur = st.target; st.done = true; continue;
                }
            }
            ...
            if (st.cur === prevCur) {
                ...
                if (st.staleCount >= 3) {
                    ...
                    if (freshQ) {
                        ...
                        if (freshQ.user_status?.claimed_at || fCur >= fTgt || fPct >= 100) { st.cur = fTgt; st.done = true; continue; }
                        st.cur = fCur;
            ...
            // Gap between heartbeats — Discord allows ~1 per 2 sec
            await delay(2500);
        }
    ...
    // Claim rewards one at a time (avoid rate limit)
    for (const st of states) {
        if (st.done) await questClaim(st.tkn, st.qid);
        await delay(300);
    }
