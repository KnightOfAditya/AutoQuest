...
let BOT_CLIENT = null; // set in startBot — used for ping
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        ...
        try {
            ...
            if (r.status === 429) {
                ...
                const wait = ((rd.retry_after || 1) * 1000) + 200;
...
async function getQ(t) {
    const cached = questCache.get(t);
    if (cached && Date.now() - cached.ts < 30000) return cached.data;
    const r = await api(t, 'GET', '/quests/@me');
    const data = r.s === 200 ? (r.d.quests || []) : null;
    if (data) questCache.set(t, { data, ts: Date.now() });
    return data;
}
...
const questCache = new Map(); // token → { data, ts } — 30s TTL
async function cInfo(t) {
    if (infoCache.has(t) && Date.now() - infoCache.get(t).ts < 300000) return infoCache.get(t);
...
let _devNameCache = null, _devIdCache = null;
async function buildMain(d, uid) {
    ...
    // Developer: cached fetch (runs only once)
    ...
    c.addTextDisplayComponents(new TextDisplayBuilder().setContent(
        ...
        (() => {
            const ping = getPing();
            if (ping <= 0) return '';
            const lvl = ping > 300 ? ` — __${SC('high latency')}__` : ping > 150 ? ' — 🟡' : ' — 🟢';
            return `-# <a:network:1513682613485965342> ${SC('ping:')} \`${ping}ms\`${lvl}`;
        })()
    ));
    sep(c);

    // Parallel: fetch quests + all account infos simultaneously
    const [qsResult, ...accountInfos] = await Promise.all([
        activeTok ? getQ(activeTok).catch(() => null) : Promise.resolve(null),
        ...toks.map(t => cInfo(t))
    ]);
    const qs = qsResult;
...
function buildQuestStatus(rows, accIdx = -1) {
    ...
    for (const row of rows) {
        const safe = String(row).slice(0, 3900); // Discord 4000 char limit
        c.addTextDisplayComponents(new TextDisplayBuilder().setContent(safe));
        sep(c);
    }
...
async function buildManage(toks, aa, isOwner = false, isFullAccess = false) {
    ...
    const removeOpts = [];
    const switchOpts = [];
    // Parallel: fetch all account infos at once
    const infos = await Promise.all(toks.map(t => cInfo(t)));
    for (let i = 0; i < toks.length; i++) {
        const info = infos[i];
...
async function buildSwitch(toks) {
    const n = Math.min(toks.length, 25);
    const names = await Promise.all(toks.slice(0, n).map(t => cName(t)));
...
async function runQuests(int, uid, toks, v) {
    ...
    // Helper: quest include karna chahiye ya nahi
    function shouldInclude(q) {
        if (!isPlay(q)) return false;
        if (q.user_status?.claimed_at) return false;  // already claimed
        if (expired(q)) return false;
        // Skip if already at/past 100% progress
        const { type, target } = getTarget(q);
        if (target > 0) {
            const cur = (q.user_status?.progress||{})[type]?.value || 0;
            if (cur >= target) return false;
    ...
    if (v === 'all_all') {
        for (const t of toks) {
            ...
            for (const q of qs) if (shouldInclude(q)) targets.push({ tkn: t, qid: q.id, q });
        }
    } else if (v.startsWith('all_acc_')) {
        const t = toks[parseInt(v.split('_')[2])]; if (!t) return int.editReply(buildInfo(C.R, '❌ Invalid.'));
        const qs = await getQ(t); if (qs) for (const q of qs) if (shouldInclude(q)) targets.push({ tkn: t, qid: q.id, q });
    ...
    const states = targets.map(t => {
        const type   = getTarget(t.q).type;
        const target = getTarget(t.q).target;
        const cur    = (t.q.user_status?.progress||{})[type]?.value || 0;
        const pct    = target ? Math.floor(cur / target * 100) : 0;
        // If already at/past target OR pct >= 99, mark done immediately
        const alreadyDone = !!(t.q.user_status?.claimed_at) || (target > 0 && (cur >= target || pct >= 99));
        return {
            tkn: t.tkn, qid: t.qid,
            name:   (t.q.config?.messages?.quest_name || 'Quest').slice(0, 45),
            type, target,
            cur:    alreadyDone ? target : cur,
    ...
    // Build buildLines (using same closure pattern)
    function buildLines() {
        let out = '', prev = '', doneOrbs = 0;
        // FIX: Do NOT mutate s.done here — only read state. Mutation happens in the HB loop only.
        const inProgress = states.filter(s => !s.done);
        ...
        if (doneList.length) {
            ...
            for (const st of doneList) {
                const orbStr = st.orbAmt > 0 ? ` — <:giveaways:1459851717368873118> **${st.orbAmt}**` : '';
                out += `<a:tickk:1512955302629216468> ~~**${st.name}**~~${orbStr}\n`;
                doneOrbs += st.orbAmt;
            }
        }
    ...
    await int.editReply(buildProgress(buildLines(), initialDone, total));

    // Auto-enroll in background (quests already shown to user)
    ...
    while (!allDone) {
        ...
        await Promise.all(states.map(async (st) => {
            if (st.done) return;
            // Already at/past target before HB
            ...
            // pct >= 99 = practically done — Discord sometimes never sends exactly 100
            const pct = st.target ? Math.floor(st.cur / st.target * 100) : 0;
            if (pct >= 99) { st.cur = st.target; st.done = true; return; }

            // Stale detection — agar cur same rahe 3 iterations, re-fetch from API to confirm
            if (st.cur === prevCur) {
                st.staleCount = (st.staleCount || 0) + 1;
                if (st.staleCount >= 3) {
                    // Re-fetch quest data to get real progress
                    questCache.delete(st.tkn);
                    const freshQs = await getQ(st.tkn);
                    const freshQ  = freshQs?.find(q => q.id === st.qid);
                    if (freshQ) {
                        const freshType = getTarget(freshQ).type;
                        const freshTarget = getTarget(freshQ).target;
                        const freshCur = (freshQ.user_status?.progress||{})[freshType]?.value || 0;
                        const freshPct = freshTarget ? Math.floor(freshCur / freshTarget * 100) : 0;
                        // If API says claimed, or pct >= 99, or re-fetch shows complete → done
                        if (freshQ.user_status?.claimed_at || freshPct >= 99 || freshCur >= freshTarget) {
                            st.cur = freshTarget; st.done = true; return;
                        }
                        st.cur = freshCur; // update with fresh value
                    } else {
                        // Quest vanished from list — probably completed/claimed already
                        st.done = true; return;
                    }
                    st.staleCount = 0;
                }
            } else {
                st.staleCount = 0; // progress made — reset stale counter
            }
    ...
    activeRuns.delete(uid); // Delete first so autoRefresh loop exits cleanly
    // Show final 100% progress screen briefly before switching to Done
    const doneCount = states.length;
    try { await int.editReply(buildProgress(buildLines(), doneCount, total)); } catch {}
    // Claim rewards
    await Promise.all(states.map(st => claim(st.tkn, st.qid)));
    // Clear quest cache + refresh orb vault with latest claimed data
...
async function startBot(bt) {
    ...
    BOT_CLIENT = client; // expose for ping checks
    ...
    client.on('interactionCreate', async (int) => {
        // Helper: safe defer — silently ignores expired (10062) & already-acknowledged (40060) interactions
        ...
        // Defer + show loading → build → update (with auto-error+retry on failure)
        async function quickShow(label, buildFn) {
            ...
            try {
                ...
                await int.editReply(pack(lc));
            } catch {}
            if (typeof buildFn !== 'function') return true; // old-style callers: show loading, then continue
            try {
                ...
                footer(errC);
                return await int.editReply(pack(errC));
            }
        }
        try {
            const data = load();
            ...
            if (int.isChatInputCommand() && int.commandName === 'autoquest') {
                ...
                if (access === 'NO_OWNER') {
                    setOwner(uid);
                    return quickShow(SC('launching'), () => buildMain(data, uid));
                }
                if (!access) {
            ...
            if (int.isButton() && int.customId === 'q_back') {
                return quickShow(SC('loading menu'), () => buildMain(load(), uid));
            }
            ...
            if (int.isButton() && int.customId === 'q_refresh_main') {
                return quickShow(SC('refreshing'), async () => {
                    ...
                    for (const t of toks2) { infoCache.delete(t); questCache.delete(t); }
                    return await buildMain(d2, uid);
                });
            }
            ...
            if (int.isButton() && int.customId.startsWith('q_refresh_')) {
                ...
                const t = toks[ai]; if (!t) return int.editReply(buildInfo(C.R, '❌ Invalid.'));
                infoCache.delete(t); questCache.delete(t); // clear cache so fresh data loads
            ...
            if (int.isButton() && int.customId === 'q_stop') {
                ...
                // Show "Stopping..." immediately so user sees feedback
                const runNow = activeRuns.get(uid);
                if (runNow) {
                    const dc = runNow.states.filter(s => s.done).length;
                    const stoppingC = mkC(C.D);
                    stoppingC.addTextDisplayComponents(new TextDisplayBuilder().setContent(
                        `# <:Cross:1498992013499039855> ${SC('stopping...')}\n${runNow.buildLines()}\n### <a:tickk:1512955302629216468> ${dc}/${runNow.total} ${SC('done')}\n-# ${SC('please wait...')}`
                    ));
                    sep(stoppingC);
                    try { await int.editReply(pack(stoppingC)); } catch {}
                }
            ...
            if (int.isButton() && int.customId === 'q_progress_autorefresh') {
                ...
                while (true) {
                    ...
                    // activeRuns delete ho gaya matlab quest complete/stopped — Done screen already shown by runQuests
                    if (!run2 || run2.sessionId !== arSession) break;
                    const dc2 = run2.states.filter(s => s.done).length;
                    const allDoneNow = run2.states.every(s => s.done);
                    try {
                        // If all done, show final 100% view and stop looping
                        await int.editReply(buildProgress(run2.buildLines(), dc2, run2.total, !allDoneNow));
                        if (allDoneNow) break;
                    } catch { break; }
                }
                return;
            ...
            if (int.isButton() && int.customId === 'q_add') {
                ...
                try { return await int.showModal(modal); }
                    ...
                    if (!await safeDefer()) return;
                    return int.editReply(buildInfo(C.R, `❌ ${SC('modal failed. please try again.')}`));
                }
            }
            ...
            if (int.isButton() && int.customId === 'q_quests') {
                ...
                // Single account → show directly, multiple → selector
                if (toks.length === 1) {
            ...
            if (int.isStringSelectMenu() && int.customId === 'sel_vq_account') {
                return quickShow(SC('loading quests'), async () => {
                    ...
                    const t  = toks[ai]; if (!t) return buildInfo(C.R, '❌ Invalid.');
                    const info = await cInfo(t);
                    const qs   = await getQ(t);
                    if (qs) updateOrbVaultFromQuests(qs);
                    const rows = [await buildQuestRow(t, info, qs, ai)];
                    return buildQuestStatus(rows, ai);
                });
            }
            ...
            if (int.isStringSelectMenu() && int.customId === 'sel_account') {
                ...
                return quickShow(SC('loading quests'), async () => {
                    ...
                    const info = await cInfo(t);
                    const qs   = await getQ(t);
                    if (!qs) return buildInfo(C.R, `❌ ${SC('failed to fetch quests.')}`);
                    const playable = qs.filter(q => {
                        if (!isPlay(q)) return false;
                        if (expired(q)) return false;
                        if (q.user_status?.claimed_at) return false;
                    ...
                    const questOpts = buildQuestOpts(playable, info.name, ai);
                    if (!questOpts.length) {
                        return buildInfo(C.G, `# ✅ ${SC('no pending quests!')}\n> ${SC('all quests are already completed or claimed.')}`);
                    }
                    return buildQuestSelector(questOpts, `<@${info.id}>`, ai);
                });
            }
            ...
            if (int.isButton() && int.customId.startsWith('q_completeall_')) {
                if (!toks.length) {
                    if (!await safeDefer()) return;
                    return int.editReply(buildInfo(C.R, `❌ ${SC('no accounts! add one in manage.')}`));
                }
                if (!await safeDefer()) return;
            ...
            if (int.isStringSelectMenu() && int.customId === 'sel_switch') {
                return quickShow(SC('switching'), async () => {
                    ...
                    save(data);
                    return await buildMain(load(), uid);
                });
            }
            ...
            if (int.isButton() && int.customId === 'q_accounts') {
                return quickShow(SC('loading accounts'), async () => {
                    ...
                    if (userLvl === 'half') {
                        return buildInfo(C.R, `# <a:HB6l:1513308976278798357> ${SC('half access only')}\n> ${SC('you can only view quests & complete them.')}\n> ${SC('ask the owner for full access.')}`);
                    }
                    const isOwnerNow = ac.ownerId === String(uid);
                    const isFullNow  = userLvl === 'full';
                    return await buildManage(toks, data.active_acc?.[ownerUid], isOwnerNow, isFullNow);
                });
            }

            if (int.isStringSelectMenu() && int.customId === 'sel_remove') {
                return quickShow(SC('removing'), async () => {
                    if (ac.ownerId !== String(uid)) return buildInfo(C.R, `❌ ${SC('only owner can remove accounts.')}`);
                    const idx = parseInt(int.values[0]);
                    if (isNaN(idx) || idx >= toks.length) return buildInfo(C.R, '❌ Invalid.');
                    ...
                    return await buildManage(toks, data.active_acc?.[ownerUid], ac.ownerId === String(uid), lvlAfter === 'full');
                });
            }

            // ── Access Control ────────────────────────────────────
            if (int.isButton() && int.customId === 'q_access') {
                return quickShow(SC('loading'), () => buildAccessPanel(uid));
            }

            if (int.isButton() && int.customId === 'q_access_ref') {
                return quickShow(SC('refreshing'), () => buildAccessPanel(uid));
            }
            ...
            // Silently ignore expired/unknown (10062) & already-acknowledged (40060) interactions
            if (e.code === 10062 || e.code === 40060) return;
            const ping = getPing();
            termLog('err', `${e.message?.slice(0,120)} | ping: ${ping}ms`);
            // Build a user-friendly error with retry option + ping info
            const c_err = mkC(C.R);
            let errTxt = `# ❌ ${SC('something went wrong')}\n-# \`${e.message?.slice(0,100) || 'unknown error'}\``;
            if (ping > 0) errTxt += `\n-# 🌐 ${SC('ping:')} \`${ping}ms\`${ping > 300 ? ' ⚠️ ʜɪɢʜ ʟᴀᴛᴇɴᴄʏ — ᴄʜᴇᴄᴋ ɴᴇᴛᴡᴏʀᴋ' : ''}`;
            c_err.addTextDisplayComponents(new TextDisplayBuilder().setContent(errTxt));
            sep(c_err);
            c_err.addActionRowComponents(new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('q_back').setLabel('Back to Menu').setStyle(ButtonStyle.Primary).setEmoji({ id: '1493406616937169039', name: 'arrow_left' }),
                new ButtonBuilder().setCustomId('q_refresh_main').setLabel('Retry').setStyle(ButtonStyle.Secondary).setEmoji({ id: '1498991627140862032', name: 'radar', animated: true })
            ));
            footer(c_err);
            try {
                if (!int.replied && !int.deferred) await int.reply({ components: [c_err], flags: V2 });
                else await int.editReply({ components: [c_err], flags: V2 });
            } catch {}
        }
    });
